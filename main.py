...
def train(model, model_path, train_loader, dev_loader, test_loader,iemo_loader,
    ...
    for e in range(n_epochs):
        ...
        test_fscores, test_reports = valid('TEST', model, test_loader, log)
        iemocap_fscores, iemocap_reports = valid('IEMOCAP', model, iemo_loader, log)
...
def main(args, seed=0, index=0):
    ...
    model = CauseDagNoEmotion(utter_dim,emo_emb,emotion_dim,att_dropout,mlp_dropout,pag_dropout,ff_dim,nhead,window,num_bases)
...
if __name__ == '__main__': 
    ...
    parser.add_argument('--gpu_list', type=str, required=False, default='0')  #required:可选参数是否可以省略
    ...
    parser.add_argument('--window', type=int, required=False, default=7)
    ...
    parser.add_argument('--seed', nargs='+', type=int, required=False, default=[1, 5, 2, 3, 4])
